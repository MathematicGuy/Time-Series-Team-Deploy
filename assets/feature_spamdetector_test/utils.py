...
def check_model_files_exist(file_path=config):
	...
	#? Check config.FILE_PATHS in config.py
    existing_files = [f for f in config.FILE_PATHS if os.path.exists(f)]
    missing_files = [f for f in config.FILE_PATHS if f not in existing_files]

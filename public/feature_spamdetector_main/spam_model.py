...
class SpamClassifier:
    def __init__(self, model_name="intfloat/multilingual-e5-base"):
        ...
        self.train_metadata: Optional[List[Dict[str, Any]]] = None
    ...
    def _load_model(self):
        ...
        if self.tokenizer is None or self.model is None:
            self.tokenizer = cast(PreTrainedTokenizerBase, AutoTokenizer.from_pretrained(self.model_name))
            self.model = cast(PreTrainedModel, AutoModel.from_pretrained(self.model_name))
    ...
    def _get_model_components(self) -> tuple[PreTrainedTokenizerBase, PreTrainedModel]:
    ...
    def _load_data_from_kaggle(self):
        ...
        df = kagglehub.load_dataset(
            getattr(KaggleDatasetAdapter, "pandas"),
    ...
    def _compute_saliency_scores(self, query_text, k=10):
        ...
        tokenizer, model = self._get_model_components()
        if self.index is None or self.train_metadata is None:
            raise ValueError("model index or metadata not ready")
        ...
        for i, token in enumerate(tokens):
            token_mask: List[str] = list(tokens)
            pad_token = tokenizer.pad_token or ""
            token_mask[i] = str(pad_token)
    ...
    def _classify_with_weighted_knn(self, query_text, query_embedding=None, k=10, alpha=0.5, explain=False):
        ...
        tokenizer, model = self._get_model_components()
        if self.index is None or self.train_metadata is None or self.class_weights is None:
            raise ValueError("model index, metadata, or class weights not ready")
        ...
        # get prediction
        predicted_label = max(vote_scores, key=lambda k: vote_scores[k])
